...
function Fruits(props) {
  ...
  const pageNumbers = Array.from({length:endPage - startPage + 1}, (_,i)=> startPage+i);
    ...
    const loadData = () => {
      axios
        .get('https://port-0-backend-mbiobnhr0088e901.sel4.cloudtype.app/fruits')
        .then(res => {
          setData(res.data);
  ...
  return (
    <section>
      ...
      <p style={{ marginTop: '20px',textAlign:'center',width:'700px' }}>
        {/* 이전 버튼 들어가는 곳 */}
        {currentPage > 1 &&(
          <button 
      ...
      <p style={{textAlign:'right',width:'700px'}}>
        <button onClick={()=> navigate('/fruits/create')}>상품등록</button>
      </p>
